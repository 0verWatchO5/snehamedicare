  ...
  return (
    <section id="hospitals" className="py-24 px-4 sm:px-6 lg:px-8 relative">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Search & Filter Bar */}
        <div className="bg-slate-900/70 border border-slate-800 backdrop-blur-2xl rounded-3xl p-4 sm:p-6 mb-10 shadow-2xl">
          <div className="flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="relative w-full md:flex-1">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-slate-950/80 border border-slate-800 rounded-xl text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-cyan-500 transition-colors"
              />
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredHospitals.map((hosp) => (
            <div
              key={hosp.id}
              className="rounded-3xl p-6 bg-slate-900/50 border border-slate-800 hover:border-cyan-500/40 backdrop-blur-xl transition-all duration-300 flex flex-col justify-between group hover:shadow-2xl hover:shadow-cyan-500/5"
            >
              <div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {hosp.specialties.map((spec, i) => (
                      <span
                        key={i}
                        className="text-[11px] px-2.5 py-0.5 rounded-lg bg-slate-800/70 text-slate-300 border border-slate-700/50"
                      >
                        {spec}
                      </span>
                    ))}
                ...
                {/* TPA Desk Notice */}
                <div className="text-[11px] text-slate-400 bg-cyan-950/30 border border-cyan-900/40 p-2.5 rounded-xl mb-6 flex items-start gap-1.5">
                  <ShieldCheck className="w-3.5 h-3.5 text-cyan-400 shrink-0 mt-0.5" />
                  <span><strong>Dedicated Helpdesk:</strong> {hosp.tpaDesk}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800">
                <a
                  href={`tel:${hosp.phone}`}
                  className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-white transition-colors"
                >
        ...
        {filteredHospitals.length === 0 && (
          <div className="text-center py-16 bg-slate-900/40 rounded-3xl border border-slate-800">
            ...
            <div className="text-white font-bold text-base mb-1">No networked hospitals match your search</div>
